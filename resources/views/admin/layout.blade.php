...
<head>
    ...
    <title>LIVESOSTORY.CO — @yield('title', 'Admin')</title>
...
<body class="bg-dark-950 text-white font-sans antialiased">
    <div class="flex min-h-screen">
        <!-- Sidebar -->
        <aside id="sidebar"
            class="fixed inset-y-0 left-0 z-50 w-64 bg-dark-900 border-r border-dark-800 transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
            <div class="flex flex-col h-full">
                <!-- Logo -->
                <div class="px-6 h-16 flex items-center border-b border-dark-800 bg-dark-950">
                    <a href="/"
                        class="text-gold-400 tracking-ultra-wide text-xs font-light uppercase">LIVESOSTORY.CO</a>
                </div>
                ...
                <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                    ...
                    <a href="{{ route('admin.categories.index') }}"
                        class="flex items-center gap-3 px-3 py-2.5 rounded text-sm {{ request()->routeIs('admin.categories.*') ? 'bg-gold-400/10 text-gold-400' : 'text-gray-400 hover:text-white hover:bg-dark-800' }} transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                ...
                <div class="px-4 py-4 border-t border-dark-800">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 bg-gold-400/20 rounded-full flex items-center justify-center">
                            <span
                                class="text-gold-400 text-xs font-semibold">{{ substr(auth()->user()->name, 0, 1) }}</span>
                        </div>
        ...
        <div class="flex-1 lg:ml-64">
            ...
            <header
                ...
                <h1 class="text-sm font-light tracking-widest uppercase text-gray-400">@yield('header', 'Dashboard')
                </h1>
            </header>

            <!-- Page Content -->
            <main class="px-4 py-6 sm:p-6 text-white overflow-x-hidden">
                @if(session('success'))
                    <div
                        class="mb-6 p-4 bg-green-500/10 border border-green-500/20 text-green-400 text-sm rounded flex items-center gap-2">
